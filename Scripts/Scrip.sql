...
CREATE TABLE tbl_factura (
  ...
  FOREIGN KEY (pk_id_almacen) REFERENCES tbl_almacen(pk_codigo_almacen),
...
CREATE TABLE IF NOT EXISTS tbl_venta (
  ...
  CONSTRAINT idcliente_v
    ...
    REFERENCES tbl_almacen (pk_codigo_almacen)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE if not exists tbl_CuentaPorCobrar(
  ...
  FOREIGN KEY (fk_id_almacen_venta) REFERENCES tbl_almacen(pk_codigo_almacen),
  FOREIGN KEY (fk_id_almacen_pago) REFERENCES tbl_almacen(pk_codigo_almacen),
...
/*--Tablas necesarias para el Consultorio--*/
